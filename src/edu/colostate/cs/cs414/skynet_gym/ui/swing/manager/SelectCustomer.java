...
public class SelectCustomer extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3371332911265243491L;
	...
	public SelectCustomer(final JTabbedPane frame) {
		...
		submitBtn.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent arg0) {
				matchingCustomers.setModel(new MyListModel(matchingCustomers()));
			}
		});
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent arg0) {
				int index = matchingCustomers.getSelectedIndex();
				
				if (index >= 0 &&
						index <= customerList.size()) {
					Customer modC = customerList.get(index);
					JPanel modCustomer = new ModifyCustomer(frame, modC);
					frame.addTab("Modify Customer: " + modC.getPersonInfo().getLastName(),
							null,
							modCustomer,
							null);
	...
	public ArrayList<String> matchingCustomers(){
